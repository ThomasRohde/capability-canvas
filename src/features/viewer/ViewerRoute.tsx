      ...
      <header className="cc-toolbar">
        ...
        <button
          ...
          onClick={() => {
            ...
            if (bounds.w > 0) setViewport({ zoom: 1, x: 40 - bounds.x * viewport.zoom, y: 40 - bounds.y * viewport.zoom });
          }}
        >
          Fit
        </button>
        ...
        >
          Heatmap <span className={`cc-toggle ${doc.heatmap.enabled ? 'on' : ''}`} />
        </button>
        <button
          className="cc-btn"
          type="button"
          onClick={() => void Promise.resolve(adapterFor('svg').exportDocument(doc)).then(saveExportResult)}
        >
        ...
        <button
          ...
          type="button"
          onClick={() => {
            localStorage.setItem('capability-canvas.import', JSON.stringify(serializeDocument(doc)));
