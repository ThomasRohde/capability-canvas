...
describe('parent containment', () => {
  ...
  it('grows ancestors after commands that place children outside parent bounds', () => {
    const added = runTransaction(createSampleDocument(), addChild('digital', 'Fourth digital capability')).doc;
  ...
  it('repairs existing documents without moving children', () => {
    ...
    expect(repaired.nodesById['digital-sales']).toMatchObject({ x: before.x, y: before.y, w: before.w, h: before.h });
  ...
  it('uses document padding settings when growing parents', () => {
    ...
    doc.nodesById.root = createNode({ id: 'root', label: 'Root', type: 'root', x: 50, y: 50, w: 10, h: 10 });
    doc.nodesById.child = createNode({ id: 'child', label: 'Child', parentId: 'root', x: 120, y: 140, w: 80, h: 40 });
...
function findContainmentViolations(doc: CapabilityDocument): string[] {
  ...
  for (const parent of Object.values(doc.nodesById)) {
    for (const childId of childrenOf(doc, parent.id)) {
      ...
      if (!child) continue;
      if (!contains(parent.id, doc, childId)) out.push(`${parent.id}->${childId}`);
    }
  }
  return out;
}

function contains(parentId: NodeId, doc: CapabilityDocument, childId: NodeId): boolean {
...
function findSiblingOverlaps(doc: CapabilityDocument): string[] {
  ...
  for (const parentId of Object.keys(doc.childrenByParentId)) {
    const childIds = childrenOf(doc, parentId === ROOT_PARENT_ID ? null : parentId);
    for (let a = 0; a < childIds.length; a += 1) {
      for (let b = a + 1; b < childIds.length; b += 1) {
        ...
        if (first && second && overlaps(first, second)) out.push(`${first.id}<->${second.id}`);
      }
    }
  }
  return out;
